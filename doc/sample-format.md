...
These metadata artifacts have distinct roles:

- `recording.toml` indexes generated media and its native timeline, following
  [Recording and Sequence Documents](recording-format.md).
- `session-record.jsonl` preserves lifecycle and provenance evidence, following
  [Session Record Format](session-record-format.md).
