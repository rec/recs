...
def test_protocol_parses_daemon_hello() -> None:
    message = gui_protocol.parse_message('{"type":"hello","role":"daemon","version":6}')
...
def test_gui_listener_replies_to_supported_hello() -> None:
    connection = FakeConnection(['{"type":"hello","role":"gui","version":6}\n'])
    listener = gui_ipc.GuiListener(connection, lambda event: None)

    listener._read()

    assert connection.sent == ['{"type":"hello","role":"daemon","version":6}\n']
...
def test_gui_listener_returns_direct_response_after_hello() -> None:
    ...
    assert connection.sent == [
        '{"type":"hello","role":"daemon","version":6}\n',
        (
