...
def test_card_replace_uses_new_session_without_changing_output_directory(
    ...
    assert rec.cfg.directory.output_directory == str(output)
    assert rec.session_directory.parents[4] == new / 'recs'
...
def test_empty_template_output_directory_record_uses_time_template(
    ...
    assert Path(
        'sessions/2026/06/23/-default-/2026/06/23/20-34-10/session-record.jsonl'
    ).exists()
...
def test_default_output_directory_uses_session_timestamp(
    ...
    rec = Recorder(Cfg(include=['Mic'], silent=True))
    rec._start_record()
    expected = Path('-default-/2026/06/23/20-34-10')
    path = rec.session_directory / 'audio/mic.wav'
...
def test_daemon_default_output_directory_uses_largest_external_disk(
    ...
    assert recording_paths.session_directory(str(large / 'takes'), timestamp) == (
        large / 'takes' / '-default-/2026/06/23/20-34-10'
    )
    assert recording_paths.media_session_directory(
        large / 'takes' / '-default-/2026/06/23/20-34-10', 'midi'
    ) == (large / 'takes' / '-default-/2026/06/23/20-34-10' / 'midi')
...
def test_sessions_on_one_day_share_a_day_directory(tmp_path: Path) -> None:
    ...
    assert first_path.parent == second_path.parent == tmp_path / '-default-/2026/06/23'
    assert recording_paths.session_directory(str(tmp_path), next_day).parent == (
        tmp_path / '-default-/2026/06/24'
    )


def test_session_directory_uses_project_name(tmp_path: Path) -> None:
    timestamp = datetime(2026, 6, 23, 20, 34, 10).timestamp()

    assert (
        recording_paths.session_directory(str(tmp_path), timestamp, 'x18-show')
        == tmp_path / 'x18-show/2026/06/23/20-34-10'
    )
