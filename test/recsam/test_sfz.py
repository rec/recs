...
def test_read_sfz_inheritance_and_common_opcodes(tmp_path: Path) -> None:
    ...
    assert instrument.body.settings.sustain is not None
    assert instrument.body.settings.sustain.control == 'sustain'
    assert instrument.body.settings.controls['sustain'].default == 0
...
def test_write_sfz_is_deterministic_and_round_trips_complete_import(
    ...
    assert second.instrument.body.settings == first.instrument.body.settings
...
def test_write_sfz_with_no_representable_slot_has_only_generated_comment() -> None:
    ...
    assert not exported.complete
    assert exported.contents == '// Generated by Ufor\n'
    ...
    document = document.model_copy(
        update={
            'body': document.body.model_copy(
                update={
                    'settings': document.body.settings.model_copy(
                        update={
                            'controls': {'expression': controls.ControlDeclaration()},
                            'sustain': None,
                        }
                    )
                }
...
def _instrument(
    *,
    ...
    return instrument.SampleInstrumentScore(
        name='test',
        ...
        body=instrument.SampleInstrument(
            settings=instrument.SampleSettings(
