...
arrangement, tuning, scale, and oscillator documents.
...
regressions remain unchanged. Ufor's GitHub CI passes. Automated checks do not
...
Finish the envelope/LFO event and state model, then settle the small final
instrument contract before moving the recsam instrument types. Sparse tuning
maps, Scala keyboard mapping, MTS byte import, oscillator phase/retrigger state,
and richer cross-domain graphs remain future model work. No sampler engine,
