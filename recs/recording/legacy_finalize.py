...
def prepare_legacy_recording(
    ...
    if not entries or not isinstance(entries[0], legacy.SessionHeader):
        raise RecsError('Recording finalization requires a version 3 session header')
    header = entries[0]
    footer = (
        entries[-1]
        if isinstance(entries[-1], legacy.SessionFooter) and not errors
        else None
    )
    starts = [
        e
        for e in entries
        if isinstance(e, legacy.FileRecord) and e.type == 'file_started'
    ]
    finishes = [
        e
        for e in entries
        if isinstance(e, legacy.FileRecord) and e.type == 'file_finished'
    ]
    ...
    return RecordingDocument(
        ...
        body=Recording(
            ...
            continued_at=[
                ...
                for e in entries
                if isinstance(e, legacy.EventRecord) and e.continued_at
            ],
        ),
    ), notes
